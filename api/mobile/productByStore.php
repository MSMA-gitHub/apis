<?php 
    require 'db.php'; 
    $sql="SELECT name,image,id,price  FROM product where storeid = ? and   product.country in( select country from users where users.id = ?) ";
  
$stmt=$conn->prepare($sql);
$stmt->execute(array($_GET['store'],$_GET['user_id']));
